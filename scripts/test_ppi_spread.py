...
# 剪刀差：PPI同比 - CPI同比（日频 ffill 后），再 z20
p = hs._morning_series("us_ppi_yoy"); q = hs._morning_series("us_cpi_yoy")
spread = (p - q).dropna()
base = spread.rolling(20).mean(); sig = spread.rolling(20).std()
feat["ppi_cpi_spread_z20"] = ((spread - base) / sig).where(sig > 0, np.nan)
...
c = close.reindex(dates).ffill(); fwd = {n: (c.shift(-n)/c - 1.0) for n in (10, 20)}
...
WIN = {"2020-2024": ("2020-01-01","2024-12-31"), "2025+": ("2025-01-01",None), "2026": ("2026-01-01",None),
       "2026-03+": ("2026-03-01",None), "8月至今": ("2026-08-01",None), "6-7月": ("2026-06-01","2026-07-31")}
def sel(s, w):
    ...
    return s
def ic(sig_, w, n):
    d = pd.DataFrame({"s": sel(sig_, w), "f": sel(fwd[n], w)}).dropna()
    return d["s"].corr(d["f"], method="spearman") if len(d) > 20 else float("nan")
print("剪刀差 z20 IC:", " | ".join("%s %+.2f/%+.2f" % (w, ic(feat["ppi_cpi_spread_z20"], w, 10), ic(feat["ppi_cpi_spread_z20"], w, 20)) for w in WIN))
def score_with(wts):
    rf = sum(w * feat[n].fillna(0.0) for n, w in wts.items() if n in feat)
...
def bt(score, start, end=None):
    ...
    def wf2(i):
        pp = pv.iloc[i]
        return (0.0, 1.0) if pp != pp else (float(pp), 1.0 - float(pp))
    rot = simulate_rotation(o, ho, hc, wf2, np.zeros(n, bool), np.ones(n), fee=5e-4,
                            ...
                            waterfall=True, hedge_allow=allow, emerg_buy=(0.04,1.5), emerg_sell=(0.035,3.0))
    return perf(rot.set_index("date")["equity"])
W = dict(V9.FLOW_WEIGHTS)
print()
for tag, wts in [("基准", W), ("剪刀差 -0.04", norm({**W, "ppi_cpi_spread_z20": -0.04})),
                 ("剪刀差 +0.04", norm({**W, "ppi_cpi_spread_z20": +0.04})),
                 ("剪刀差 +0.08", norm({**W, "ppi_cpi_spread_z20": +0.08}))]:
    sc = score_with(wts)
    print("%-14s %s" % (tag, " | ".join("%s %+6.1f%%/%.2f" % (wn, 100*bt(sc, a, b)["cum"], bt(sc, a, b)["calmar"]) for wn, (a, b) in WIN.items())))
