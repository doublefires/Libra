# -*- coding: utf-8 -*-
"""CPI 权重敏感性：-0.08(现行)/-0.05/-0.03/0/+0.05/+0.08（其余权重同比例归一）。"""
import os, sys
...
def score_with(w):
    rf = sum(v * feat[n].fillna(0.0) for n, v in w.items() if n in feat)
...
print("=" * 108)
print("%-14s | %-26s | %-26s | %-26s | %-26s" % ("CPI权重", "2020-2024", "2025+", "2026", "2026-03+"))
for cpi in (-0.08, -0.05, -0.03, 0.0, +0.05, +0.08):
    s = score_with(norm({**W, "us_cpi_yoy_z20": cpi}))
    line = "%-14s" % ("%+.2f" % cpi)
    for w in WIN:
    ...
    print(line)
print()
print("%-14s %8s %8s %8s %8s %8s" % ("CPI权重", "25+10", "25+20", "26-03+10", "26-03+20", "20-24:20"))
for cpi in (-0.08, -0.05, -0.03, 0.0, +0.05, +0.08):
    s = score_with(norm({**W, "us_cpi_yoy_z20": cpi}))
    print("%-14s %+8.3f %+8.3f %+8.3f %+8.3f %+8.3f" % ("%+.2f" % cpi, ic(s, "2025+", 10), ic(s, "2025+", 20),
