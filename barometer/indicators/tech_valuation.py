"""⑦ 科技股估值（tech_valuation）—— 科技股现在是不是已经太贵？（永远用分位数而非绝对 PE）

指标清单与方向定义见 config/modules.py 注册表（ids_of_module('tech_valuation')）。
"""
...
def compute_snapshots(engine, as_of_date: str) -> list:
    ...
    """
    out = []
    for iid in mcfg.ids_of_module("tech_valuation"):
